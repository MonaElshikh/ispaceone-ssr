...
@Component({
  selector: 'app-spouses-couples-details',
  templateUrl: './spouses-couples-details.component.html',
  styleUrls: ['./spouses-couples-details.component.css']
})
export class SpousesCouplesDetailsComponent implements OnInit , OnDestroy {
  ...
  constructor(
    private SpousesCouplesService: SpousesCouplesService,
  ...
  fillAllArticles() {
    ...
    //fill the main list
    this.listSubscribe = this.SpousesCouplesService.getList()
      .subscribe((data) => {
        this.SpousesCouplesService.parseXML(data)
          .then((data) => {
            ...
            if (this.title) {
              this.TagsResourcesObject = this.SpousesCouplesService.getByTitle(this.articles, this.title);
  ...
  fillReadthisAndRelatedArticles(title: string, list: any[]) {
    //clear lists
    this.SpousesCouplesService.clearList(this.readThisList);
    this.SpousesCouplesService.clearList(this.articlesList);
    this.SpousesCouplesService.clearList(this.extraList);
    this.extraList = this.SpousesCouplesService.getReadthisAndRelatedArticles(list ,title).reverse();
